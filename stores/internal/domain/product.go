...
func (p *Product) ApplyEvent(event ddd.Event) error{
	...
	default:
		return errors.ErrInternal.Msgf("%T received the event %s with unexpectted payload %T", p, event.EventName(), payload)
...
func (p *Product) ApplySnapshot(snapshot es.Snapshot) error{
	...
	default:
		return errors.ErrInternal.Msgf("%T recived the unexpected snapshot %T", p, snapshot)
	}
