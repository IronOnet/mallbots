...
var (
	...
	ErrBasketCannotBeCanceled   = errors.Wrap(errors.ErrBadRequest, "the basket cannot be canceled")
	ErrQuantityCannotBeNegative = errors.Wrap(errors.ErrBadRequest, "the quantity cannot be negative")
...
func (b Basket) ApplyEvent(event ddd.Event) error {
	...
	default:
		return errors.ErrInternal.Msgf("%T received the event with an expected payload %T", b, event.EventName(), payload)
	}
...
func (b *Basket) ApplySnapshot(snapshot es.Snapshot) error {
	...
	default:
		return errors.ErrInternal.Msgf("%T received the unexpected snapshots %T", b, snapshot)
	}
