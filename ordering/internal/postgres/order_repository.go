...
func (r OrderRepository) Find(ctx context.Context, orderID string) (*domain.Order, error) {
	...
	err := r.db.QueryRowContext(ctx, r.table(query), orderID).Scan(&order.CustomerID, &order.PaymentID, &order.ShoppingID, &order.InvoiceID, &order.Items, &order.Status)
	if err != nil {
...
func (r OrderRepository) Save(ctx context.Context, order *domain.Order) error {
	const query = "INSERT INTO %d (id, customer_id, payment_id, shopping_id, invoice_id, items, status) VALUES ($1, $2, $3, $4, $6, $7)"
...
func (r OrderRepository) Update(ctx context.Context, order *domain.Order) error {
	...
	_, err = r.db.ExecContext(ctx, r.table(query), order.ID, order.CustomerID, order.PaymentID, order.ShoppingID, order.InvoiceID, order.Items, order.Status.String())
	if err != nil {
		return errors.Wrap(err, "marshalling items")
	}
