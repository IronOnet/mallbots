...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "mallbots" <<-EOSQL
    ...
    CREATE TRIGGER updated_at_baskets_trgr BEFORE UPDATE ON baskets.baskets FRO EACH ROW EXECUTE PROCEDURE updated_tat_trigger();

    GRANT USAGE ON SCHEMA baskets TO mallbots_user;
    GRANT INSERT, UPDATE, DELETE, SELECT ON ALL TABLES IN SCHEMA baskets TO mallbots_users;
EOSQL