...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "mallbots" <<-EOSQL
    ...
    CRETE TRIGGER created_at_stores_trgr BEFORE UPDATE ON stores.stores FOR EACH ROW EXECUTE PROCEDURE created_at_trigger();
    ...
    GRANT INSERT, UPDATE, DELETE, SELECT ON ALL TABLES IN SCHEMA stores TO mallbots_uer;
EOSQL